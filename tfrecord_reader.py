...
class TFRecordReader:
    ...
    def __init__(self, target_to_index, api_to_index, arg_to_index, config, is_evaluating=False):
        ...
        if file_prefix is not None and len(self.filenames) == 0:
            print('%s cannot find files: %s.*' % ('Evaluation reader' if is_evaluating else 'Train reader', file_prefix))
        ...
        with tf.name_scope('TFRecordReader'):
            ...
            # if session is not None and not tf.executing_eagerly():
            #     self.subtok_table.init.run(session=session)
            #     self.node_table.init.run(session=session)
    ...
    def process_dataset(self, parsed_context, parsed_example):
        ...
        adj_matrix = tf.scatter_nd(indices=edges, updates=tf.ones(tf.shape(edges)[0], dtype=tf.float32), shape=[num_nodes, num_nodes]) # (n, n)
        ...
        dense_api_strings = tf.sparse.to_dense(sp_input=sparse_api_strings, default_value=common.blank_target_padding) # (n, api_max_name_parts)
        ...
        clipped_api_lengths = tf.clip_by_value(tf.cast(api_lengths, dtype=tf.int32), clip_value_min=0, clip_value_max=api_max_name_parts)
        ...
        clipped_arg_lengths = tf.clip_by_value(tf.cast(arg_lengths, dtype=tf.int32), clip_value_min=0, clip_value_max=max_args)
    ...
    def get_padded_shapes(self):
        return {PACKAGE: [],
                ...
                TARGET_NAME_IDS: [None], #[self.config.TARGET_MAX_PARTS + 1],
                TARGET_NAME_LENGTHS: [],
    ...
    def _create_filtered_output(self):
        dataset = tf.data.TFRecordDataset(self.filenames, buffer_size=self.config.CSV_BUFFER_SIZE,
                                          num_parallel_reads=None, compression_type=tf.io.TFRecordOptions.compression_type_map[tf.io.TFRecordCompressionType.GZIP])
        ...
        #                         lambda context, example: tf.equal(context['filepath'],
        #                                                           b'file:///home/urialon/Allamanis-dataset/java-small/validation/libgdx121/BufferedParticleBatch.java'))
        #dataset = dataset.filter(predicate=lambda context, example: tf.equal(context['line'], 36))
        ...
        #                           padding_values=self.get_default_values())
        ...
        #bucket_batch_sizes = [self.batch_size, self.batch_size]
...
if __name__ == '__main__':
    #subtok_to_index = {PAD: 0, UNK: 1, 'a': 2, 'b': 3, 'c': 4, 'd': 5}
    #node_to_index = {PAD: 0, UNK: 1, 'A': 2, 'B': 3, 'C': 4, 'D': 5}
    ...
    with open('{}.dict'.format(config.TRAIN_PATH), 'rb') as file:
        ...
        arg_name_to_index, index_to_arg_name, arg_names_vocab_size = \
        common.load_vocab_from_dict(arg_to_count, 1, start_from=0, add_values=[common.blank_target_padding])
